...
import { playSound, playBackgroundMusic, stopBackgroundMusic } from '../sounds.js'
import {
  loadFont,
  loadImage,
  getImageData,
...
export default class DinoGame extends GameRunner {
  constructor(width, height) {
    ...
    // Individual images for dinosaur, cactus, and bird
    this.dinoImage = null
    this.cactusImage = null
    this.birdImage = null
  ...
  async preload() {
    ...
    // load sprite sheet + per-actor individual images
    const [
      spriteImage,
      dinoImage,
      cactusImage,
      ...
      loadImage('./assets/dinosaur.png'),
      loadImage('./assets/cactus.png'),
      loadImage('./assets/bird.jpg'),
      loadImage('./assets/amitabh.png'),
      loadImage('./assets/amma.png'),
      loadImage('./assets/cid.png'),
    ])

    await loadFont('./assets/PressStart2P-Regular.ttf', 'PressStart2P')
    this.spriteImage = spriteImage
    this.spriteImageData = getImageData(spriteImage)
    this.dinoImage = dinoImage
    this.cactusImage = cactusImage
    // keep fallback birdImage for compatibility
    this.birdImage = birdImage
    // store images per variant for per-instance painting
    this.birdImages = {
      bird: birdImage,
      amitabh: amitabhImage,
      amma: ammaImage,
      cid: cidImage,
    }
  ...
  resetGame() {
    ...
    this.start()
    stopBackgroundMusic() // Stop and reset music to beginning
    playBackgroundMusic() // Start music from beginning
  }
  ...
  drawBirds() {
    ...
    if (this.frameCount % settings.birdSpawnRate === 0) {
      ...
      if (randBoolean()) {
        // choose a variant for this bird
        const variants = ['bird', 'amitabh', 'amma', 'cid']
        const variant = variants[randInteger(0, variants.length - 1)]
        const newBird = new Bird(this.spriteImageData, {
          variant,
          image: this.birdImages[variant],
        })
        ...
        birds.push(newBird)
        // Play the variant-specific spawn sound directly from the bird instance
        newBird.playSpawnSound()
      }
    }
  ...
  paintInstances(instances) {
    for (const instance of instances) {
      // pass the instance so paintSprite can use per-instance images (e.g., bird variants)
      this.paintSprite(instance.sprite, instance.x, instance.y, instance)
    }
  }

  paintSprite(spriteName, dx, dy, instance) {
    // Check if this is a dinosaur, cactus, or bird sprite and use individual images
    if (spriteName.startsWith('dino')) {
      // Use dinosaur image for all dinosaur sprites
      const { h, w } = sprites[spriteName]
      this.canvasCtx.drawImage(this.dinoImage, dx, dy, w / 2, h / 2)
      return
    }

    if (spriteName.startsWith('cactus')) {
      // Use cactus image for all cactus sprites
      const { h, w } = sprites[spriteName]
      this.canvasCtx.drawImage(this.cactusImage, dx, dy, w / 2, h / 2)
      return
    }

    if (spriteName.startsWith('bird')) {
      // Use per-instance bird image if present, otherwise fallback to default
      const { h, w } = sprites[spriteName]
      const img = instance && instance.image ? instance.image : this.birdImage
      this.canvasCtx.drawImage(img, dx, dy, w / 2, h / 2)
      return
    }
    
    // For all other sprites, use the sprite sheet
