...
export default class DinoGame extends GameRunner {
  ...
  async preload() {
    ...
    const [spriteImage, dinoImage, cactusImage, birdImage] = await Promise.all([
  ...
  paintSprite(spriteName, dx, dy) {
    ...
    if (spriteName.startsWith('bird')) {
      // Use bird image for all bird sprites
      const { h, w } = sprites[spriteName]
      this.canvasCtx.drawImage(this.birdImage, dx, dy, w / 2, h / 2)
      return
    }
